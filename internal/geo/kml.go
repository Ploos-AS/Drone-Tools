...
type kmlPlacemark struct {
	...
	Polygon *struct {
		Outer struct {
			Ring kmlGeometry `xml:"LinearRing"`
		} `xml:"outerBoundaryIs"`
	} `xml:"Polygon"`
}
...
func ParseKML(r io.Reader) (Summary, error) {
	...
	if err := xml.NewDecoder(r).Decode(&doc); err != nil {
		return Summary{}, fmt.Errorf("decode KML: %w", err)
	}
	...
	for _, p := range placemarks {
		...
		if p.Point != nil {
			a.summary.Points++
			if err := addKMLCoords(&a, p.Point.Coordinates); err != nil {
				return Summary{}, err
			}
		}
		if p.Line != nil {
			a.summary.LineStrings++
			if err := addKMLCoords(&a, p.Line.Coordinates); err != nil {
				return Summary{}, err
			}
		}
		if p.Polygon != nil {
			a.summary.Polygons++
			if err := addKMLCoords(&a, p.Polygon.Outer.Ring.Coordinates); err != nil {
				return Summary{}, err
			}
		}
	}
...
func addKMLCoords(a *accumulator, raw string) error {
	for _, token := range strings.Fields(raw) {
		...
		if len(parts) < 2 {
			return fmt.Errorf("invalid KML coordinate %q", token)
		}
