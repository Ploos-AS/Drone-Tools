...
func ParseGeoJSON(r io.Reader) (Summary, error) {
	...
	if err := json.NewDecoder(r).Decode(&root); err != nil {
		return Summary{}, fmt.Errorf("decode GeoJSON: %w", err)
	}
	...
	case "FeatureCollection":
		...
		for _, f := range root.Features {
			if f.Geometry != nil {
				if err := addGeometry(&a, *f.Geometry); err != nil {
					return Summary{}, err
				}
			}
		}
	case "Feature":
		...
		if root.Geometry != nil {
			if err := addGeometry(&a, *root.Geometry); err != nil {
				return Summary{}, err
			}
		}
	default:
		if root.Type == "" {
			return Summary{}, fmt.Errorf("missing GeoJSON type")
		}
		if err := addGeometry(&a, geoJSONGeometry{Type: root.Type, Coordinates: root.Coordinates}); err != nil {
			return Summary{}, err
		}
	}
...
func addGeometry(a *accumulator, g geoJSONGeometry) error {
	...
	case "MultiPoint":
		...
		a.summary.Points += len(coords)
		for _, c := range coords {
			if err := addPair(a, c); err != nil {
				return err
			}
		}
	case "LineString":
	...
	case "MultiLineString":
		...
		a.summary.LineStrings += len(lines)
	...
	case "MultiPolygon":
		...
		a.summary.Polygons += len(polygons)
	...
	case "GeometryCollection":
		for _, child := range g.Geometries {
			if err := addGeometry(a, child); err != nil {
				return err
			}
		}
	default:
...
func addPair(a *accumulator, c []float64) error {
	if len(c) < 2 {
		return fmt.Errorf("coordinate needs longitude and latitude")
	}
	a.addCoord(c[0], c[1])
	return nil
}
