...
func Parse(r io.Reader) (Summary, error) {
	...
	for i, p := range points {
		if p.Lat < s.Bounds.MinLat { s.Bounds.MinLat = p.Lat }
		if p.Lat > s.Bounds.MaxLat { s.Bounds.MaxLat = p.Lat }
		if p.Lon < s.Bounds.MinLon { s.Bounds.MinLon = p.Lon }
		if p.Lon > s.Bounds.MaxLon { s.Bounds.MaxLon = p.Lon }
		if p.Ele != nil {
			if s.MinElevation == nil || *p.Ele < *s.MinElevation { v := *p.Ele; s.MinElevation = &v }
			if s.MaxElevation == nil || *p.Ele > *s.MaxElevation { v := *p.Ele; s.MaxElevation = &v }
		}
		if p.Time != nil {
			if s.StartTime == nil || p.Time.Before(*s.StartTime) { v := *p.Time; s.StartTime = &v }
			if s.EndTime == nil || p.Time.After(*s.EndTime) { v := *p.Time; s.EndTime = &v }
		}
		if i > 0 { s.DistanceMeters += haversine(points[i-1], p) }
	}
...
func convertPoint(p xmlPoint) Point {
	...
	if p.Time != "" {
		if t, err := time.Parse(time.RFC3339, p.Time); err == nil { out.Time = &t }
	}
	return out
}
