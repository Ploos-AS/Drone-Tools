...
func TestParseGPX(t *testing.T) {
	...
	if doc.Format != "gpx" || len(doc.Paths) != 1 || len(doc.Paths[0]) != 2 || len(doc.Points) != 1 { t.Fatalf("unexpected document: %+v", doc) }
	if doc.Bounds == nil || doc.Bounds.MaxLat != 58.1 || doc.Bounds.MaxLon != 7.2 { t.Fatalf("unexpected bounds: %+v", doc.Bounds) }
}
...
func TestParseKML(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if doc.Format != "kml" || len(doc.Paths) != 1 || len(doc.Points) != 1 { t.Fatalf("unexpected document: %+v", doc) }
}
...
func TestParseGeoJSON(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if doc.Format != "geojson" || len(doc.Paths) != 1 || len(doc.Points) != 1 { t.Fatalf("unexpected document: %+v", doc) }
}

func TestUnsupportedFormat(t *testing.T) {
	if _, err := Parse("flight.txt", strings.NewReader("x")); err == nil { t.Fatal("expected unsupported-format error") }
}
